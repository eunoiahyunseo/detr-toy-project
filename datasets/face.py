...
'''
    img: 이는 하나의 이미지를 나타낸다. Face 데이터셋에서의 이미지는 일반적으로
...
'''
    Face 데이터셋의 세그멘테이션 정보를 마스크 형태로 변환하는 역할을 한다.
    
    함수의 입력으로 segmentations는 Face 데이터셋에서 특정 이미지의 모든 객체에 대한 폴리곤(polygon) 좌표를 담고 있는 리스트이다.
    그리고 height와 width는 해당 이미지의 높이와 너비이다.
    
    polygon은 [x1, y1, x2, y2,..., xn, yn]이런 식으로 나타내진다. 폴리곤 세그멘테이션은 일반적으로 대상 객체의 픽셀 수준에서의 정확한 위치와 형태
    이를 Face에 적용하기 위해서는 RLE(Run-Length Encoding)을 적용해주어야 하는데, 이진 마스크로 인코딩 해야 한다. 
...
def convert_Face_poly_to_mask(segmentations, height, width):
    ...
    for polygons in segmentations:
        rles = Face_mask.frPyObjects(polygons, height, width) # 각 폴리곤을 이진 마스크로 변환한다. RLE 형식으로 변환
...
'''
    Face 데이터셋 형식의 이미지와 대상(annotation)을 받아서 처리하여 적절한 형태로 변환하는 것이다.
